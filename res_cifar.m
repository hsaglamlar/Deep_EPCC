...
% if opts.preActivation ,
%     net2 = res_cifar_preactivation_init(m) ;
% else
%     net2 = res_cifar_init(m, 'networkType', opts.modelType, ...
%       'reLUafterSum', opts.reLUafterSum) ;
% end

net = res_imagenet_init(101, 'nClasses', opts.nClasses,...
...
% resized_images = zeros(224, 224, 3, size(imdb.images.data,4));
% for i = 1:size(imdb.images.data,4)
%     resized_images(:,:,:,i) = imresize(imdb.images.data(:,:,:,i), [224 224]);
% end
% imdb.images.data = single(resized_images);

imdb_loaded = load('dataset/cifar10/cifar10_imdb.mat');
imdb.images = imdb_loaded.imdb.images;
imdb.meta.dataMean = imresize(imdb.meta.dataMean, [224 224]);
