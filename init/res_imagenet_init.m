...
if opts.polyhedral
    ...
    if opts.binary
        net.addLayer('polyhedral', dagnn.Polyhedral(), {'pool_final','label'}, 'polyhedral');
    elseif opts.multilabel
        net.addLayer('polyhedral', dagnn.Polyhedral(), {'pool_final','label'}, 'polyhedral');
    else
        net.addLayer('polyhedral', dagnn.Polyhedral(), {'pool_final','label'}, 'polyhedral');
    end
    ...
    block = dagnn.Conv('size', [1 1 info.lastNumChannel*2 nClasses], 'hasBias', true, ...
                       'stride', 1, 'pad', 0);
    lName = sprintf('fc%d', info.lastIdx+1);
    net.addLayer(lName, block, 'polyhedral', lName, {[lName '_f'], [lName '_b']});
    
    if opts.binary
        net.addLayer('loss', dagnn.Loss('loss', 'polyhedral_bc'), {'fc133', 'label'}, 'loss');
    elseif opts.multilabel
        net.addLayer('loss', dagnn.Loss('loss', 'polyhedral_ml'), {'fc133', 'label'}, 'loss');
    else
        net.addLayer('loss', dagnn.Loss('loss', 'polyhedral_mc'), {'fc133', 'label'}, 'loss');
    end
    
    if opts.multilabel
        net.addLayer('error', dagnn.Loss('loss', 'classerror_onehot'), {'fc133','label'}, 'error') ;
    net.addLayer('error5', dagnn.Loss('loss', 'topkerror_onehot', 'opts', {'topK', 5}), ...
      {'fc133','label'}, 'error5') ;
    else
        net.addLayer('error', dagnn.Loss('loss', 'classerror'), {'fc133','label'}, 'error') ;
