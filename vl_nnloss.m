...
switch lower(opts.loss)
    case {'classerror', 'topkerror', 'log', 'softmaxlog', 'mhinge', 'mshinge', 'polyhedral_mc', 'polyhedral_bc'}
    ...
    case {'binaryerror', 'binarylog', 'logistic', 'hinge', 'polyhedral_ml', 'classerror_onehot', 'topkerror_onehot'}
...
switch lower(opts.loss)
    case {'log', 'softmaxlog', 'mhinge', 'mshinge'}
        % from categopts.lossory labels to indexes
...
if nargin <= 2 || isempty(dzdy)
    switch lower(opts.loss)
        case 'classerror_onehot'
            countCorrect = 0;
            noImages = size(x, 4);
            for indImage = 1:noImages
                [~, scoreInds] = sort(x(1, 1, :, indImage), 'descend');
                
                for k = 1:3
                    countCorrect = countCorrect + c(1, 1, scoreInds(k), indImage);
                end
            end
            t = 1 - (countCorrect / (noImages * 3));
            t = gpuArray(single(t));

        case 'topkerror_onehot'
            countCorrect = 0;
            noImages = size(x, 4);
            for indImage = 1:noImages
                [~, scoreInds] = sort(x(1, 1, :, indImage), 'descend');
            ...
            t = 1 - (countCorrect / (noImages * 3));
            fprintf('\n Multi label classification error rate is %.2f \n',t);
            t = gpuArray(single(t));
            
        case 'classerror'
            if size(x, 3) == 1
                t = 1 - (sum((x > 0) .* (c == 1)) + sum((x < 0) .* (c == -1))) / size(x, 4);
                t = gpuArray(single(t));
            else
                [~,chat] = max(x,[],3) ;
                t = cast(c ~= chat) ;
        ...
        case 'polyhedral_mc'
            ...
            for indImage = 1:size(scores, 4)
                for indClass = 1:size(scores, 3)
                    if labels(indImage) ~= indClass
                        score = scores(1, 1, labels(indImage), indImage) - scores(1, 1, indClass, indImage);
                        hingeScore = max(0, 1 - score);
                        loss = loss + hingeScore;
                    end
                end
            end
            loss = loss / size(scores, 4);
            t = gpuArray(single(loss));
        case 'polyhedral_bc'
            scores = gather(x);
            labels = c;
            loss = 0;
            ww1 = length(find(labels==-1));
            ww2 = length(labels)-ww1;
            ww = 7;%ww1/ww2;
            for indImage = 1:size(scores, 4)
                label = labels(indImage);
                score = label * scores(1, 1, 1, indImage);
                if label==1,
                    hingeScore = ww*max(0, 1 - score);
                else
                    hingeScore = max(0, 1 - score);
                end
                loss = loss + hingeScore;
        ...
        case 'polyhedral_ml'
            scores = gather(x);
            labels = c;
            noImages = size(scores, 4);
            loss = 0;
            
            for indImage = 1:noImages
                posClasses = find(labels(1, 1, :, indImage) == 1);
                negClasses = find(labels(1, 1, :, indImage) ~= 1);
                for indPos = 1:size(posClasses, 1)
                    posClass = posClasses(indPos);
                    m = length(negClasses);
                    jj=randperm(m);
                    q = 0;
                    for ii=1:length(jj),
                        negClass = negClasses(jj(ii));
                        q = q+1;
                        scoreDiff = scores(1, 1, posClass, indImage) - scores(1, 1, negClass, indImage);
                        if 1-scoreDiff>0,
                            break;
                        end
                    end
                    rr = floor((length(jj)-1)/q);
                    LL = 0;
                    for ii=1:rr,
                        LL =  LL + (1/ii);
                    end
                    LL =1;
                    for indNeg = 1:size(negClasses, 1)
                        negClass = negClasses(indNeg);
                        scoreDiff = scores(1, 1, posClass, indImage) - scores(1, 1, negClass, indImage);
                        loss = loss + max(0, 1 - scoreDiff)*LL;
                    end
                end
            end
            t = gpuArray(single(loss));
            ...
            for indImage = 1:size(scores, 4)
                for indClass = 1:size(scores, 3)
                    if labels(indImage) ~= indClass
                        ...
                        hingeScore = max(0, 1 - score);
                        if hingeScore > 0
                            grads(1, 1, labels(indImage), indImage) = grads(1, 1, labels(indImage), indImage) - 1;
                            grads(1, 1, indClass, indImage) = 1;
                        end
                    end
                end
            end
            y = gpuArray(single(dzdy .* grads));
        case 'polyhedral_bc'
            scores = gather(x);
            labels = c;
            grads = zeros(size(x));
            ...
            for indImage = 1:size(scores, 4)
                ...
                if hingeScore > 0
                    if label == 1
                        grads(1, 1, 1, indImage) = - label * ww ;
                    else
                        grads(1, 1, 1, indImage) = - label;
                    end
                end
            end
            y = gpuArray(single(dzdy .* grads));
            ...
            for indImage = 1:noImages
                ...
                for indPos = 1:size(posClasses, 1)
                    ...
                    for indNeg = 1:size(negClasses, 1)
                        negClass = negClasses(indNeg);
                        scoreDiff = scores(1, 1, posClass, indImage) - scores(1, 1, negClass, indImage);
                        hingeLoss = max(0, 1 - scoreDiff);
                        if hingeLoss  > 0
                            grads(1, 1, posClass, indImage) = grads(1, 1, posClass, indImage) -LL;
                            grads(1, 1, negClass, indImage) = grads(1, 1, negClass, indImage) + LL;
                        end
                    end
                end
                if 0
                for indPos = 1:size(posClasses, 1),
                    posClass = posClasses(indPos);
                    score_current = scores(1, 1, posClass, indImage);
                    hingeLoss = max(0, 1 - score_current);
                    if hingeLoss  > 0
                        grads(1, 1, posClass, indImage) = grads(1, 1, posClass, indImage) -1;
                    end
                end
                for indNeg = 1:size(negClasses, 1)
                        negClass = negClasses(indNeg);
                        score_current = scores(1, 1, negClass, indImage);
                        hingeLoss = max(0, 1 + score_current);
                        if hingeLoss  > 0
                        grads(1, 1, negClass, indImage) = grads(1, 1, negClass, indImage) +1;
                        end
                end
                end
